...
| **AI-suggested rules** | A confirmed FRAUD is mined for a standing rule (statistics only, no model needed) and backtested against real outcomes, saved as **shadow**. A human reviews and activates or dismisses it from the Rules tab. |
...
- **AI-suggested rules from a confirmed miss** (`app/rule_mining.py`): marking a case FRAUD also mines it for a standing rule. Deterministic statistics only — the same exact-binomial contrast test already used for "abnormal firing" (`app.clusters.binom_tail`) checks how rare each of the case's own signal values is against the rest of the queue, greedily AND-combines the strongest ones (PRIM/RIPPER-style), drops anything that duplicates an already-active rule, and backtests every survivor against real confirmed outcomes before it is ever saved — as **shadow**, never active. A high-cardinality field like `state` is excluded from mining entirely: with 1–2 cases per value it produces spurious "rare" patterns, not signal, and it's the same fairness proxy risk already called out below. An optional background step asks the model for a nicer name only — it cannot touch the logic, and if it's unavailable the deterministic name already saved stands. A human reviews and activates or dismisses each candidate from the Rules tab; retracting the outcome discards any that were never promoted.

### 7. Rules tab and backtests (`app/rulestore.py`, `app/backtest.py`, `app/customrules.py`, `app/rule_mining.py`)
